...
| P0 | Serve/Funnel and mobile truth | Serve and Funnel share upstream state, while TLS certificate lookup is disabled on iOS/Android. | R5 implements one first-Up/config authority, ServeConfig Funnel, bounded ETag writes, and exact handles. Hosted tailnet/swap receipts passed 2026-08-10; crash/restart proof remains. Keep `tls.bind` blocked on an alternate cert path and ServeConfig Funnel unqualified until separate device/sidecar receipts pass. |
