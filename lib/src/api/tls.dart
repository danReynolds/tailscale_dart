...
/// enabled MagicDNS + HTTPS in the admin panel; [bind] will fail with a
/// clear error if those are off. Use [domains] as a preflight to check.
class Tls {
  const Tls();

  /// Binds a TLS listener on the tailnet. The returned
  /// [SecureServerSocket] decrypts incoming traffic server-side; handlers
  /// see plaintext bytes.
