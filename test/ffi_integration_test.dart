...
void main() {
  ...
  group('duneStart error handling', () {
    test('returns valid JSON for unreachable control URL', () {
      ...
      if (parsed.containsKey('transportBootstrap')) {
        expect(
          (parsed['transportBootstrap'] as Map<String, dynamic>).containsKey(
            'masterSecretB64',
          ),
          isTrue,
        );
  ...
  group('before up()', () {
    ...
    test('udp.bind throws', () async {
      await expectLater(
        Tailscale.instance.udp.bind(1234),
        throwsA(isA<TailscaleUsageException>()),
      );
    });
  });
  ...
  group('up/down lifecycle', () {
    ...
    test(
      'status() after down() returns stopped (persisted state exists)',
      () async {
        final status = await Tailscale.instance.status();
        expect(status.state, NodeState.stopped);
        expect(status.tailscaleIPs, isEmpty);
